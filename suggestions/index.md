...
1. [Publishable website pages](./website/index.md)
   Canonical constructional contracts for a future static docs site. Stage 27 covers helper `python:3-alpine` digest pins, helper `/health` probes, CPU/IO shares, host cron JSON off `/tmp`, docker GID `group_add`, helper `deploy.resources`, and Tecnativa docker-socket-proxy.
