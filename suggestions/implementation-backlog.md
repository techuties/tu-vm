...
## P1-2: Markdown-first suggestion publishing

### Scope

Adopt [`website-markdown-publishing-system.md`](./website-markdown-publishing-system.md) as the canonical path for turning existing suggestion markdown into website-ready community pages. Start with frontmatter, generated indexes, and validation around the current `suggestions/` files before adding any new website service.

### Acceptance criteria

- Canonical suggestion files have a small, documented frontmatter schema (`title`, `status`, `category`, `tags`, and related links where useful).
- A script or CI check validates required fields and known status/category values for website-published suggestion files.
- `/suggestions/`, `/community/`, `/docs/playbooks/`, and `/roadmap/` routes are represented in the selected static docs framework plan.
- Rollback keeps all proposal content readable as plain markdown in GitHub.
...
1. **P1-2** — markdown publishing schema and indexes, because it improves community suggestions without runtime risk.
...
2. **Markdown suggestion publishing checks** — frontmatter schema validation, generated indexes, and duplicate hints for canonical suggestion pages (**P1-2**).
