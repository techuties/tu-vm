...
tags:
  ...
  - community
  - tooling
source: consolidated from historical community suggestion branches
---
...
Create a markdown-first publishing model for the TU-VM website and community suggestion system. The goal is to make proposals easy to write, review, render on a website, and trace through implementation without introducing a custom CMS or heavy governance stack.
...
1. **Primary path: Docusaurus**
   ...
   - Good plugin ecosystem for search, generated indexes, and edit links.
   - Familiar contribution flow for GitHub-based communities.

2. **Lean path: Astro with Starlight**
   - Fast static output.
   - Excellent docs UX defaults.
   - Good fit if the website later needs richer landing or marketing pages.

3. **Lightweight fallback: MkDocs Material**
   - Excellent readability and search.
   - Strong choice if maintainers prefer a Python-based docs toolchain.

Do not build a custom database-backed proposal app until markdown, GitHub Issues, and CI checks no longer meet the project's needs.
...
4. **Proposed approach** - what should change and which framework or tool should be reused.
...
- Website pages can be generated from repository markdown without manual copy and paste.
