...
description: Publishable Stage 26 community website markdown for Chromium shm_size, n8n-mcp digest pins, MCP Tier-2 restart honesty, Tika/MinIO resource limits, processor tmpfs with status share, datastore nofile ulimits, and OOM score order—extending Stages 1–25 without reinventing frameworks or community intake.
...
**Stage 26** (this run) adds **shm, pin, restart-honesty, cgroup-limit, tmpfs-share, fd-ceiling, and OOM-order constructional contracts** that remained open after Stage 1–25 drafts and consolidation PRs (#23–#51):

- Compose `shm_size` for Chromium `/dev/shm` on browserless and mcp-playwright
- Digest pin for `n8n_mcp` (still `:latest`) using the existing safe-update map
- Honest `restart: "no"` on MCP filesystem/fetch/memory
- `deploy.resources` on Tika and MinIO (the remaining unbounded Tier 1 pair)
- Processor scratch tmpfs **after** relocating the helper status file
- `ulimits.nofile` on postgres/minio, copied from Nginx
- `oom_score_adj` so Ollama is sacrificed before Pi-hole and Postgres

These are **constructional**: they tell contributors how to extend existing Compose keys, the `tu-vm.sh` pin map, and the helper/processor JSON status file without inventing a browser appliance, a second updater, a keep-alive sidecar, a memory governor, a status bus, or host PAM tuners.
...
| Page | Purpose |
|---|---|
| [Chromium shm_size](./chromium-shm-size-contract.md) | 1G `/dev/shm` on browserless and mcp-playwright |
| [n8n-mcp digest pin](./n8n-mcp-digest-pin-contract.md) | Pin the last floating official sidecar |
| [MCP Tier-2 restart honesty](./mcp-tier2-restart-honesty-contract.md) | `restart: "no"` on filesystem/fetch/memory |
| [Tika and MinIO resource limits](./tika-minio-resource-limits-contract.md) | Cgroup CPU/RAM on the unbounded Tier 1 pair |
| [Processor tmpfs and status share](./processor-tmpfs-status-share-contract.md) | Scratch tmpfs without breaking PDF progress |
| [Datastore nofile ulimits](./ulimits-nofile-datastores-contract.md) | Copy Nginx `ulimits.nofile` to postgres/minio |
| [OOM score priority](./oom-score-adj-priority-contract.md) | Kill Ollama before Pi-hole and Postgres |
...
2. Day-to-day tools → Chromium shm / n8n-mcp pin / MCP restart honesty
3. Tika-MinIO limits → processor tmpfs → datastore ulimits → OOM order
...
Extend `stage-merge-playbook.md` through **Stage 26** when reconciling hubs so sibling pages from PRs #27–#51 are not lost.
