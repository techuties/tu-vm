...
description: Publishable Stage 27 community website markdown for helper image pins, helper health probes, CPU/IO shares, host-/tmp isolation, docker GID group_add, helper resource limits, and docker-socket-proxy—extending Stages 1–26 without reinventing frameworks or community intake.
...
**Stage 27** (this run) adds **pin, probe, share, path, gid, budget, and socket-proxy constructional contracts** that remained open after Stage 1–26 drafts and consolidation PRs (#23–#52):

- Digest pin for `helper_index` (`python:3-alpine`) until the Stage 22 bake lands
- Honest Compose healthcheck against the existing helper `/health` route
- `cpu_shares` / `blkio_weight` so inference does not starve DNS and Postgres
- Host cron JSON (`tu-vm-update-status.json`, `tu-vm-log-status.json`) off host `/tmp`
- `group_add` docker GID so Stage 25 non-root helper can still talk to the socket
- `deploy.resources` on `helper_index` (last unbounded always-on first-party service)
- Tecnativa `docker-socket-proxy` instead of a raw `/var/run/docker.sock` bind

These are **constructional**: they tell contributors how to extend existing Compose keys, the `tu-vm.sh` pin map, helper `/health`, daily-checkup paths, and a mature socket-proxy image without inventing a helper distro, a second health API, a cgroup governor, a status bus, a custom socket ACL, or host PAM tuners.
...
| Page | Purpose |
|---|---|
| [Helper Python image pin](./helper-python-image-pin-contract.md) | Digest-pin `python:3-alpine` until Stage 22 bake |
| [Helper healthcheck honesty](./helper-status-healthcheck-honesty-contract.md) | Probe existing `/health` so Nginx can wait |
| [CPU shares and blkio weight](./cpu-shares-blkio-weight-contract.md) | Relative scheduler weights vs hard cgroup caps |
| [Helper host `/tmp` isolation](./helper-host-tmp-isolation-contract.md) | Cron JSON off host `/tmp` (not the Tika status file) |
| [Helper docker GID `group_add`](./helper-docker-gid-group-add-contract.md) | Non-root helper leftover from Stage 25 |
| [Helper resource limits](./helper-resource-limits-contract.md) | `deploy.resources` on `helper_index` |
| [Docker socket proxy](./docker-socket-proxy-contract.md) | Tecnativa proxy instead of a raw socket bind |
...
| Chromium shm | Compose `shm_size` | Host `/dev/shm` bind or a browser appliance |
| Sidecar pin | Existing `tu-vm.sh update` pin map | Floating `:latest` in Compose |
| Tier 2 start | `restart: "no"` + dashboard | `unless-stopped` on MCP tools |
| Tika/MinIO RAM | Compose `deploy.resources` | Unbounded Tier 1 images |
| Processor scratch | tmpfs after relocating `STATUS_FILE` | Overlaying `/tmp` and breaking PDF progress |
| Fd ceiling | Compose `ulimits.nofile` (copy Nginx) | Host PAM / systemd LimitNOFILE as default |
| OOM order | Compose `oom_score_adj` | earlyoom or `oom_kill_disable` |
| Helper image until bake | Digest pin + `OFFICIAL_UPDATE_IMAGE_PAIRS` | Floating `python:3-alpine` or a custom distro |
| Helper liveness | Existing `/health` Compose probe | Inventing a second health API or probing `/status` |
| Scheduler fairness | Compose `cpu_shares` / `blkio_weight` | A cgroup governor product or host nice daemons |
| Cron status JSON | Dedicated bind (`state/` or `/var/lib/tu-vm`) | Host `/tmp` share or a status-bus service |
| Non-root docker.sock | `group_add` docker GID | Keeping helper root “because the socket is 660” |
| Helper RAM/CPU | Compose `deploy.resources` | Unbounded first-party Flask |
| Socket blast radius | Tecnativa docker-socket-proxy | Raw socket bind or a custom ACL sidecar |
| Image CVEs / pins | Stage 4 Trivy + Stage 15 safe-update | Treating shm or OOM hints as supply-chain |
...
2. Day-to-day tools → helper pin / health probe / CPU-IO shares
3. Host `/tmp` isolation → docker GID → helper limits → socket proxy
...
Extend `stage-merge-playbook.md` through **Stage 27** when reconciling hubs so sibling pages from PRs #27–#52 are not lost.
