...
description: Publishable Stage 15 community website markdown for safe-update, n8n node-types extraction, Open WebUI first-boot init, LangGraph smoke, MCP gateway ops, docs-link CI, and MinIO bucket lifecycle contracts that extend Stages 1–14 without reinventing frameworks or community intake.
...
TU-VM already has strong operational surfaces (`tu-vm.sh`, helper API, Nginx dashboard, playbooks, CI, safe-update, MCP gateway, Open WebUI init helpers, LangGraph smoke, and MinIO-backed document flows). Historical suggestion branches repeatedly asked for the same foundations:
...
**Stage 15** (this run) adds **update, extraction, first-boot, smoke, gateway ops, docs CI, and object-lifecycle contracts** that remained open after Stage 1–14 drafts and consolidation PRs (#23–#40):

- Safe-update contribution contract (`scripts/safe-update.sh` pin map + check/apply/rollback)
- n8n node-types extraction contract (`extract-n8n-node-types.sh` → gateway JSON)
- Open WebUI init / first-boot contract (`init-openwebui.sh`)
- LangGraph supervisor smoke contract (`langgraph-e2e-smoke.sh`)
- MCP gateway ops contribution contract (`mcp-gateway/` day-2 + contribution rules)
- Docs link CI contribution contract (Lychee / Docs links workflow)
- MinIO bucket lifecycle contract (naming, retention, corpus hygiene)
...
| Stage 1–14 website pages (open PRs #27–#40) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Safe-update contribution contract](./safe-update-contribution-contract.md) | Extend pin maps and check/apply/rollback without a second updater |
| [n8n node-types extraction contract](./n8n-node-types-extraction-contract.md) | Regenerate gateway node-types JSON from local n8n |
| [Open WebUI init / first-boot contract](./open-webui-init-firstboot-contract.md) | Idempotent first-boot patching distinct from day-2 loader toggles |
| [LangGraph supervisor smoke contract](./langgraph-supervisor-smoke-contract.md) | Day-2 trust-path smoke without a parallel QA platform |
| [MCP gateway ops contribution contract](./mcp-gateway-ops-contribution-contract.md) | Auth-preserving gateway ops and tool routing contributions |
| [Docs link CI contribution contract](./docs-link-ci-contribution-contract.md) | Lychee workflow and exclude hygiene for community docs |
| [MinIO bucket lifecycle contract](./minio-bucket-lifecycle-contract.md) | Bucket naming, retention, and corpus hygiene |
...
| Image updates | `safe-update.sh` check/apply/rollback | Watchtower / floating tags by default |
| n8n intelligence metadata | extract-n8n-node-types → gateway JSON | Web scrapers as the only path |
| Open WebUI first boot | `init-openwebui.sh` idempotent repair | Hard forks as the default |
| Supervised-write smoke | `langgraph-e2e-smoke.sh` | Disabling verification to “go green” |
| Tool routing | Existing `mcp-gateway` | Shadow unauthenticated proxies |
| Docs link hygiene | Lychee Docs links workflow | Custom crawler platforms |
| Object corpora | MinIO buckets + lifecycle dry-runs | Git as the operator object store |
...
2. Day-to-day tools → hardware matrix → personas → profiles → CLI / safe-update contracts
3. MCP catalog → gateway ops → node-types extraction → LangGraph smoke → MinIO lifecycle
4. Quality gates → docs-link CI → rollout gates → release notes → merge playbook → backlog
...
Extend `stage-merge-playbook.md` through **Stage 15** when reconciling hubs so sibling pages from PRs #27–#40 are not lost.
