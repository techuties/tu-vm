...
description: Publishable Stage 20 community website markdown for tika-minio-processor Python, Nginx stub_status pairing, AFFiNE Redis durability, IPv6 dual-stack policy, systemd user-unit install, WSL2 contributor path, and n8n webhook versus login-zone split—extending Stages 1–19 without reinventing frameworks or community intake.
...
TU-VM already has strong operational surfaces (`tu-vm.sh`, helper API, Nginx dashboard, playbooks, CI, smoke/pre-push gates, Tika XML, `tika-minio-processor/`, `monitoring/` Prometheus stubs, MCP Fetch, and local backup). Historical suggestion branches repeatedly asked for the same foundations:
...
**Stage 20** (this run) adds **processor, edge-metrics, data-plane, network, and contributor-host constructional contracts** that remained open after Stage 1–19 drafts and consolidation PRs (#23–#45):

- `tika-minio-processor` Python contribution (distinct from Tika XML)
- Nginx `stub_status` pairing for the nginx Prometheus exporter
- `affine_redis` durability (named volume + explicit AOF/RDB)
- IPv6 dual-stack policy (IPv4-first)
- systemd `--user` unit install for Tier 1
- WSL2 contributor path
- n8n webhook versus login-zone split

These are **constructional**: they tell contributors how to extend existing Python, Nginx, Redis, Compose, systemd, WSL, and n8n surfaces without inventing a second document worker, metrics product, cache, overlay network, supervisor, Windows stack, or WAF.
...
| Stage 1–19 website pages (open PRs #27–#45) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Tika-MinIO processor Python](./tika-minio-processor-python-contribution-contract.md) | Watch/retry/status changes stay in the existing Alpine worker |
| [Nginx stub_status exporter pairing](./nginx-stub-status-exporter-contract.md) | Internal stub_status only, paired with opt-in nginx-exporter |
| [AFFiNE Redis durability](./affine-redis-durability-contract.md) | Named volume + explicit AOF/RDB; do not share platform Redis |
| [IPv6 dual-stack policy](./ipv6-dual-stack-policy-contract.md) | IPv4-first; dual-stack is all-or-nothing |
| [systemd user-unit install](./systemd-user-unit-install-contract.md) | Optional oneshot `--user` unit; not a second supervisor |
| [WSL2 contributor path](./wsl2-contributor-path-contract.md) | Windows contributors use WSL2 + the same bash scripts |
| [n8n webhook vs login zone](./n8n-webhook-login-zone-split-contract.md) | Split locations so login can use `zone=login` without breaking webhooks |
...
| Document worker Python | `tika-minio-processor/` universal modules | Airflow / a second OCR worker |
| Tika parsers / timeouts | Stage 19 `tika-config.xml` | Forking the Tika image |
| Nginx metrics | Internal stub_status + nginx-prometheus-exporter | Public `/nginx_status` or log shippers |
| AFFiNE Redis | Dedicated service + named volume / AOF | Sharing platform Redis or Redis Cloud |
| IPv6 | Optional dual-stack on existing `ai_network` | Second overlay / IPv6-only listeners |
| Start on login | systemd `--user` oneshot | supervisord / root system units |
| Windows contrib | WSL2 + existing `tu-vm.sh` | PowerShell control plane / `/mnt/c` clones |
| n8n login vs webhooks | Prefix locations + existing `zone=login` / `zone=api` | WAF / captcha / login zone on `location /` |
...
2. Day-to-day tools → processor Python / stub_status / Redis durability
3. IPv6 policy → systemd unit → WSL2 path
4. n8n webhook split → login rate-limit → quality gates → merge playbook
...
Extend `stage-merge-playbook.md` through **Stage 20** when reconciling hubs so sibling pages from PRs #27–#45 are not lost.
