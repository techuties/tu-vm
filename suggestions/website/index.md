...
description: Publishable Stage 21 community website markdown for Nginx Docker-DNS upstreams, platform Redis durability, helper uploader modules, multi-arch image policy, Podman/rootless path, AppArmor/seccomp profiles, and restic/borg backup backends—extending Stages 1–20 without reinventing frameworks or community intake.
...
**Stage 21** (this run) adds **edge-resolution, platform-cache, helper-module, multi-arch, alternate-runtime, LSM, and backup-backend constructional contracts** that remained open after Stage 1–20 drafts and consolidation PRs (#23–#46):

- Nginx `proxy_pass` Docker DNS names versus hardcoded `172.20.0.x`
- Platform `redis` AOF and named-volume policy (distinct from AFFiNE Redis)
- `helper/uploader.py` module contribution (distinct from the helper API contract)
- ARM64 / multi-arch image policy
- Podman / rootless Docker contributor path (distinct from WSL2)
- AppArmor / seccomp profiles (distinct from cap_drop)
- Optional restic / borg backup backend (distinct from tar-volume backup)

These are **constructional**: they tell contributors how to extend existing Nginx, Redis, Flask, Compose, Linux security, and `tu-vm.sh backup` surfaces without inventing a second reverse proxy, cache product, helper service, image registry, container runtime, LSM stack, or backup CLI.
...
| Stage 1–20 website pages (open PRs #27–#46) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Nginx Docker-DNS upstreams](./nginx-upstream-dns-contract.md) | Prefer Compose service names with an explicit resolver; keep static IPs as a fallback |
| [Platform Redis durability](./platform-redis-durability-contract.md) | Make `redis_data` + AOF/RDB an explicit policy for `ai_redis` |
| [Helper uploader modules](./helper-uploader-module-contribution-contract.md) | Split `uploader.py` into importable modules without a second Flask app |
| [Multi-arch image policy](./multi-arch-image-policy-contract.md) | Digest pins that work on amd64 and arm64 without a second compose file |
| [Podman / rootless path](./podman-rootless-contributor-path-contract.md) | Optional Docker-compatible runtime lane; keep Compose as source of truth |
| [AppArmor / seccomp profiles](./apparmor-seccomp-profile-contract.md) | Optional LSM profiles on top of existing resource limits |
| [restic / borg backup backend](./restic-borg-backup-backend-contract.md) | Optional backend behind `tu-vm.sh backup` / `restore` |
...
| Edge upstreams | Compose DNS + `resolver 127.0.0.11` | Traefik / Caddy / hardcoded-IP-only forever |
| Platform Redis | Official Redis AOF + existing `redis_data` | KeyDB / Redis Cloud / sharing `affine_redis` |
| Helper modules | Split `helper/` packages imported by `uploader.py` | FastAPI rewrite or a second control API |
| Multi-arch | Multi-arch digests + optional `platform:` | A second `docker-compose.arm64.yml` |
| Alternate runtime | `DOCKER_HOST` / compose v2 compatible Podman | Forking every script to `podman-compose` |
| LSM hardening | Distro AppArmor/seccomp + Compose `security_opt` | Custom LSM kernel modules |
| Backup backends | restic or borg behind `tu-vm.sh backup` | Replacing the command with a new CLI |
...
2. Day-to-day tools → Nginx DNS upstreams / platform Redis / helper modules
3. Multi-arch policy → Podman path → AppArmor/seccomp
4. restic/borg backend → local backup contract → quality gates → merge playbook
...
Extend `stage-merge-playbook.md` through **Stage 21** when reconciling hubs so sibling pages from PRs #27–#46 are not lost.
