...
description: Publishable Stage 9 community website markdown for product-surface and day-to-day community contracts that extend Stages 1–8 without reinventing frameworks or community intake.
...
TU-VM already has strong operational surfaces (`tu-vm.sh`, helper API, Nginx dashboard, playbooks, CI, MCP gateway, LangGraph supervisor, n8n, MinIO, Qdrant, Ollama, AFFiNE, Pi-hole, rclone mounts, monitoring, update/rollback). Historical suggestion branches repeatedly asked for the same foundations:
...
**Stage 9** (this run) adds **product-surface and day-to-day community contracts** that remained open after Stage 1–8 drafts and consolidation PRs (#23–#34):

- Privacy-preserving usage analytics and recommendations (CHANGELOG potential improvement)
- Ollama / local model catalog contribution path
- AFFiNE collaboration workspace contribution contract
- Pi-hole DNS hygiene contribution contract
- Offsite backup / rclone remote contribution contract
- Dashboard feature-flag experiments contract
- Dashboard release-highlights (“What is new”) contract
...
| Stage 1–8 website pages (open PRs #27–#34) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Privacy-preserving usage analytics](./privacy-preserving-usage-analytics.md) | Local-only usage patterns and recommendations without SaaS telemetry |
| [Ollama model catalog contribution](./ollama-model-catalog-contribution.md) | Community path for recommending and documenting local models |
| [AFFiNE collaboration contribution contract](./affine-collaboration-contribution-contract.md) | Safe community recipes around the existing AFFiNE workspace |
| [Pi-hole DNS hygiene contribution](./pihole-dns-hygiene-contribution.md) | Day-to-day LAN DNS / allowlist hygiene without a second DNS product |
| [Offsite backup rclone contract](./offsite-backup-rclone-contract.md) | Optional remote backup lane reusing rclone + existing backup commands |
| [Dashboard feature-flag experiments](./dashboard-feature-flag-experiments.md) | Env-driven UI experiments without a feature-flag platform |
| [Dashboard release highlights](./dashboard-release-highlights-contract.md) | Optional “What is new” bullets from Releases/CHANGELOG on the LAN dashboard |
...
| Usage insights | Local counters / helper summaries | Cloud product analytics by default |
| Local models | Ollama + documented catalog YAML | Custom model registry SaaS |
| Collaboration notes | AFFiNE already in Compose | Building a second wiki product |
| LAN DNS hygiene | Pi-hole + existing allowlist flows | Parallel DNS stacks |
| Offsite backups | rclone remotes over existing `backup` artifacts | Mandatory cloud DR control planes |
| UI experiments | Env/feature flags on existing dashboard | Third-party experimentation platforms |
| Release highlights | GitHub Releases / CHANGELOG | Scraping arbitrary third-party blogs into the LAN UI |
...
- Community → Insights → privacy-preserving usage analytics
- Community → Models → Ollama model catalog
- Community → Collaborate → AFFiNE contribution contract
- Community → Network → Pi-hole DNS hygiene
- Community → Reliability → offsite rclone backup (+ Stage 6 local drill)
- Community → Dashboard → feature flags + release highlights (+ Stage 4 modularization smoke)
- Community → Lifecycle → Stage 8 smart startup / updates / secrets / RAG / canary
- Community → Operate daily → Stage 5 task runner + Dev Container + health digest
- Operate → Playbooks (existing `docs/playbooks/`)

## Success signals

- Usage recommendations stay local and never require cloud telemetry accounts
- Community model advice lands as catalog/docs PRs instead of ad-hoc chat dumps
- AFFiNE recipes reuse the existing service instead of proposing another wiki
- Pi-hole hygiene contributions have a clear review lane and evidence checklist
- Optional offsite backups reuse rclone + `tu-vm.sh backup` without a new DR product
- Dashboard experiments ship behind flags with documented rollback
- “What is new” can show three bullets with a graceful static-link fallback

Canonical vocabulary and lifecycle states live in [`../website-community-pages.md`](../website-community-pages.md). Active backlog priority is tracked in [`../implementation-backlog.md`](../implementation-backlog.md). Merge Stage hubs using the Stage 4 `stage-merge-playbook.md` (expected sibling)—extend that playbook’s merge order to include Stages 5–9 before treating any single stage `index.md` as final.
