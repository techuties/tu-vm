...
description: Publishable Stage 18 community website markdown for Compose IPAM, resource budgets, MCP tool sandboxes, beginner quickstart, local backup/restore, helper control auth, and container hardening contracts that extend Stages 1–17 without reinventing frameworks or community intake.
...
**Stage 18** (this run) adds **platform-construction contracts** that remained open after Stage 1–17 drafts and consolidation PRs (#23–#43):

- Compose IPAM address registry (static `ai_network` IPv4 uniqueness)
- Compose resource-budget contract (laptop-friendly CPU/memory limits)
- MCP tool sandbox contribution contract (Playwright / filesystem / fetch / memory)
- Beginner quickstart contribution contract (`./tu-vm.sh quickstart`)
- Local backup/restore contribution contract (`backup` / `restore` / rotation)
- Helper control-auth contract (`CONTROL_TOKEN` + 401 + dashboard token)
- Container hardening baseline (capabilities, docker.sock, non-root where safe)

These are **constructional**: they tell contributors how to extend Compose, MCP tools, onboarding, backups, and control-plane auth without inventing a second orchestrator, VPN, or community API.
...
| Stage 1–17 website pages (open PRs #27–#43) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Compose IPAM address registry](./compose-ipam-address-registry.md) | Unique static IPv4 allocation on `172.20.0.0/16` so new services cannot collide |
| [Compose resource-budget contract](./compose-resource-budget-contract.md) | Energy-aware CPU/memory limits for community Compose changes |
| [MCP tool sandbox contribution contract](./mcp-tool-sandbox-contribution-contract.md) | Safe community path for Playwright, filesystem, fetch, and memory MCP images |
| [Beginner quickstart contribution contract](./beginner-quickstart-contribution-contract.md) | Portable-default onboarding without flipping silent full-stack starts |
| [Local backup/restore contribution contract](./local-backup-restore-contribution-contract.md) | `tu-vm.sh backup` / `restore` format, rotation, and secret-safe archives |
| [Helper control-auth contract](./helper-control-auth-contract.md) | `CONTROL_TOKEN`, header-first auth, and unauthenticated 401 behavior |
| [Container hardening baseline](./container-hardening-baseline-contract.md) | cap_drop / no-new-privileges / docker.sock review for Compose PRs |
...
| Static IPs | Compose IPAM + uniqueness CI | DHCP-style guessing / duplicate `.30` assignments |
| Energy budgets | Existing `deploy.resources` limits | Unlimited containers “for convenience” |
| MCP tools | Existing `mcp-tools/*` images + catalog | Ad-hoc host `npx` MCP servers |
| First-hour setup | `./tu-vm.sh quickstart` portable default | Silent `--server` as the beginner path |
| Local restore | `tu-vm.sh backup` / `restore` | A second backup product before retention exists |
| Control mutations | `CONTROL_TOKEN` header + allowlist | Open docker.sock proxies |
| Hardening | Additive `cap_drop` / `no-new-privileges` | Privileged mode as a community default |
...
2. Day-to-day tools → IPAM registry / resource budgets / MCP sandboxes / quickstart
3. Backup/restore → control-auth → container hardening → helper contract-check
...
Extend `stage-merge-playbook.md` through **Stage 18** when reconciling hubs so sibling pages from PRs #27–#43 are not lost.
