...
description: Publishable Stage 28 community website markdown for pull_policy, remaining Tier 2 budgets, n8n healthz, Compose extension fields, AFFiNE Redis auth, n8n Postgres DNS, and Compose include splits—extending Stages 1–27 without reinventing frameworks or community intake.
...
**Stage 28** (this run) adds **pull, leftover-budget, probe, DRY, auth, DNS, and include constructional contracts** that remained open after Stage 1–27 drafts and consolidation PRs (#23–#53):

- `pull_policy: missing` on digest-pinned official images
- `deploy.resources` on the remaining unbounded Tier 2 services
- Enable the commented n8n `/healthz` probe when that service is started
- Compose `x-*` extension fields and YAML anchors for repeated DNS/logging blocks
- `requirepass` on `affine_redis` (auth, not Stage 20 durability)
- n8n Postgres host via the `postgres` service name, not `172.20.0.10`
- Compose `include:` so the 880-line monolith can be split without a second orchestrator

These are **constructional**: they tell contributors how to extend existing Compose keys, the `tu-vm.sh` pin map, n8n's own `/healthz`, Compose spec extension fields, and AFFiNE/n8n env names without inventing a pull daemon, a resource governor, a second health API, a Compose preprocessor, a shared Redis, hardcoded IPAM clients, or a custom stack splitter.
...
Do **not** republish Stage 11 `container-log-retention-contract.md` (Compose `logging` max-size) or Stage 22 `logging-driver-journald-contract.md` as a new logging product. Stage 18 `container-hardening-baseline-contract.md` already covers `security_opt: no-new-privileges` — implement that in code, do not write another page.

## Page map (Stage 28)

| Page | Purpose |
|---|---|
| [Pull policy on digest pins](./pull-policy-missing-digest-contract.md) | `pull_policy: missing` so day-to-day `up` does not re-pull |
| [Unbounded Tier 2 resource limits](./unbounded-tier2-resource-limits-contract.md) | `deploy.resources` leftover after Stages 26–27 |
| [n8n healthz when started](./n8n-healthz-when-started-contract.md) | Enable the commented `/healthz` probe |
| [Compose extension fields DRY](./compose-extension-fields-dry-contract.md) | `x-*` anchors for repeated DNS (and later logging) |
| [AFFiNE Redis requirepass](./affine-redis-requirepass-contract.md) | Auth parity with platform Redis |
| [n8n Postgres service DNS](./n8n-postgres-service-dns-contract.md) | `DB_POSTGRESDB_HOST: postgres` not a static IP |
| [Compose include split](./compose-include-split-contract.md) | Official `include:` instead of a custom splitter |
...
| Image pull on daily `up` | Compose `pull_policy: missing` | A pull daemon or Watchtower-on-start |
| Remaining Tier 2 RAM | Compose `deploy.resources` | Unbounded n8n / AFFiNE / MCP / LangGraph |
| n8n liveness | Existing `/healthz` Compose probe | A custom n8n watchdog or `/status` scrape |
| Repeated Compose keys | Official `x-*` + YAML anchors | A Jinja/Jsonnet Compose preprocessor |
| AFFiNE Redis auth | `--requirepass` + AFFiNE password env | Sharing platform Redis or leaving it open |
| n8n → Postgres | Compose service DNS (`postgres`) | Hardcoded `172.20.0.10` |
| File size / ownership | Compose `include:` + CODEOWNERS | A custom stack splitter or monorepo generator |
| Container log retention | Stage 11 `logging:` max-size | A new logging product or Loki default |
| Journald opt-in | Stage 22 logging-driver contract | Rewriting json-file as Stage 28 |
| Privilege drop | Stage 18 `no-new-privileges` **code** | Another hardening essay |
| Image CVEs / pins | Stage 4 Trivy + Stage 15 safe-update | Treating `pull_policy` as supply-chain |
...
2. Day-to-day tools → pull policy / leftover Tier 2 budgets / n8n healthz
3. Compose DRY anchors → AFFiNE Redis auth → n8n Postgres DNS → include split
...
Extend `stage-merge-playbook.md` through **Stage 28** when reconciling hubs so sibling pages from PRs #27–#53 are not lost.
