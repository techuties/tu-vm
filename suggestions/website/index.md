...
description: Publishable Stage 23 community website markdown for Nginx snippet configs, MinIO versioning/ILM, read-only proxy rootfs, Postgres GUCs/pooling, Qdrant HNSW/quantization, host timezone/NTP, and inter-service network isolation—extending Stages 1–22 without reinventing frameworks or community intake.
...
**Stage 23** (this run) adds **edge-config, object-lifecycle, immutable-proxy, Postgres-GUC, vector-memory, host-clock, and east-west-network constructional contracts** that remained open after Stage 1–22 drafts and consolidation PRs (#23–#48):

- Nginx `include` snippets (optional Compose `configs:`) instead of copied TLS blocks
- MinIO versioning and ILM via existing `mc` helpers
- Read-only rootfs plus tmpfs for stateless proxies
- Real Postgres `statement_timeout` / GUC flags (optional PgBouncer later)
- Qdrant HNSW and quantization inside the 1G budget
- One operator timezone and host NTP (no time container)
- Role networks so data stores are not on every bridge

These are **constructional**: they tell contributors how to extend existing Nginx includes, `minio/mc`, Compose `read_only`, official Postgres flags, Qdrant config, host `timedatectl`, and Compose bridges without inventing a reverse-proxy product, a second object store, a custom hardened image, a clustered database, a second vector engine, a time appliance, or a service mesh.
...
| Page | Purpose |
|---|---|
| [Nginx Compose configs](./nginx-compose-configs-contract.md) | Shared TLS/header snippets; optional Compose `configs:` |
| [MinIO versioning / ILM](./minio-object-versioning-ilm-contract.md) | Opt-in object history via existing `mc` |
| [Read-only rootfs proxies](./read-only-rootfs-proxy-contract.md) | `read_only` + tmpfs for Nginx, not LSM |
| [Postgres GUCs / pooling](./postgres-statement-timeout-pooling-contract.md) | Real `-c` flags; optional PgBouncer later |
| [Qdrant HNSW / quantization](./qdrant-hnsw-quantization-contract.md) | Fit RAG in the 1G limit without a second vector DB |
| [Host timezone / NTP](./host-timezone-ntp-contract.md) | One `TU_VM_TZ`; host clock; no time container |
| [Inter-service isolation](./inter-service-network-isolation-contract.md) | Role bridges beyond a single `ai_network` |
...
| Shared Nginx config | Official `include` snippets | A templating language or new proxy |
| Object history | `minio/mc` version + ILM | A second object store |
| Immutable proxy | Compose `read_only` + tmpfs | A custom hardened Nginx image |
| Postgres GUCs | Official `postgres -c` | Fake `POSTGRES_*` env as config |
| Vector memory | Qdrant HNSW / quantization | Chroma or raising limits first |
| Time | One env + host timedatectl | A chrony container |
| East-west isolation | Extra Compose bridges | A service mesh |
| Image CVEs / pins | Stage 4 Trivy + Stage 15 safe-update | Treating isolation or ILM as supply-chain |
...
2. Day-to-day tools → Nginx snippets / MinIO ILM / read-only rootfs
3. Postgres GUCs → Qdrant memory → timezone → role networks
...
Extend `stage-merge-playbook.md` through **Stage 23** when reconciling hubs so sibling pages from PRs #27–#48 are not lost.
