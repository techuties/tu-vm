...
description: Publishable Stage 12 community website markdown for edge-access, diagnostics, and operator-communication day-to-day contracts that extend Stages 1–11 without reinventing frameworks or community intake.
...
TU-VM already has strong operational surfaces (`tu-vm.sh`, helper API, Nginx dashboard, playbooks, CI, UFW access modes, `doctor`/`diagnose`, announcements, DNS client helpers, `env.example`). Historical suggestion branches repeatedly asked for the same foundations:
...
**Stage 12** (this run) adds **edge-access, diagnostics, and operator-communication community contracts** that remained open after Stage 1–11 drafts and consolidation PRs (#23–#37):

- Access mode and firewall contract (`secure` / `public` / `locked`)
- Nginx edge routing contract (vhost and proxy contribution rules)
- Env schema contribution contract (`env.example` evolution)
- Diagnostics and triage contract (`doctor` / `diagnose` / smoke)
- Dashboard announcements contract (priority taxonomy and helper rules)
- LAN DNS client onboarding contract (`dns-clients` / router first hour)
- Host port binding contract (53 / 80 / 443 conflict hygiene)
...
| Stage 1–11 website pages (open PRs #27–#37) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Access mode and firewall contract](./access-mode-and-firewall-contract.md) | Safe community path for `secure` / `public` / `locked` and UFW |
| [Nginx edge routing contract](./nginx-edge-routing-contract.md) | Vhost, proxy, and conf contribution rules at the edge |
| [Env schema contribution contract](./env-schema-contribution-contract.md) | Evolving `env.example` without secret leakage |
| [Diagnostics and triage contract](./diagnostics-and-triage-contract.md) | `doctor` / `diagnose` / smoke as day-to-day triage tools |
| [Dashboard announcements contract](./dashboard-announcements-contract.md) | Priority taxonomy for helper-driven operator alerts |
| [LAN DNS client onboarding contract](./lan-dns-client-onboarding-contract.md) | Router and client first-hour DNS recipes |
| [Host port binding contract](./host-port-binding-contract.md) | Port 53 / 80 / 443 conflict detection and recovery |
...
| Host firewall | Existing `secure` / `public` / `locked` UFW helpers | Mandatory cloud WAF or second firewall product |
| Edge routing | Existing `nginx/conf.d` + TLS + allowlist | Replacing Nginx with an application framework for routing |
| Env evolution | Commented `env.example` + `check-config` / `generate-secrets` | Committing live `.env` or inventing a secrets SaaS |
| Triage | `doctor` / `diagnose` / smoke / privacy-safe support bundle | Pasting production secrets into public Issues |
| Operator alerts | Helper `/announcements` priority taxonomy | Push notifications to third-party SaaS by default |
| DNS onboarding | `dns-clients` + Pi-hole/Tailscale playbooks | Requiring every client to install a custom app |
| Port conflicts | Detection in `doctor`/`diagnose` + documented remediations | Silent bind failures without operator guidance |
...
When reconciling Stage 1–12 hubs, extend `stage-merge-playbook.md` (Stage 4) so the union `index.md` lists every sibling once.
