...
description: Publishable Stage 22 community website markdown for helper image bake, platform Postgres WAL/PITR, image provenance/cosign, Compose secrets, Qdrant snapshots, journald logging, and helper Compose watch—extending Stages 1–21 without reinventing frameworks or community intake.
...
**Stage 22** (this run) adds **helper-image, Postgres-durability, provenance, secrets-mount, vector-snapshot, log-driver, and contributor-sync constructional contracts** that remained open after Stage 1–21 drafts and consolidation PRs (#23–#47):

- Bake `helper_index` OS/Python deps instead of apk+pip on every start
- Platform Postgres dump-only vs opt-in WAL/PITR (distinct from Redis AOF)
- Cosign / provenance verify of digest pins (distinct from Trivy and safe-update)
- Compose `secrets:` versus plaintext `.env` (distinct from env-schema and rotation)
- Qdrant snapshot/compaction day-2 (distinct from data-plane hygiene)
- json-file versus journald log driver (distinct from max-size retention)
- Helper bind-mount / Compose Watch (distinct from Dev Container)

These are **constructional**: they tell contributors how to extend existing Dockerfiles, Postgres, Sigstore, Compose secrets, Qdrant, Docker logging, and bind-mounts without inventing a second helper API, database product, registry, secret vault, vector store, log stack, or live-reload PaaS.
...
| Stage 1–21 website pages (open PRs #27–#47) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Helper index image bake](./helper-index-image-bake-contract.md) | Bake apk/pip into `helper/Dockerfile`; keep the bind-mount |
| [Platform Postgres WAL/PITR](./postgres-wal-pitr-policy-contract.md) | Name dump-plus-volume as default; opt-in WAL behind `tu-vm.sh` |
| [Image provenance / cosign](./image-provenance-cosign-contract.md) | Verify digest pins without replacing Trivy or safe-update |
| [Compose secrets](./compose-secrets-contribution-contract.md) | Optional `secrets:` / `*_FILE`; `.env` stays the clone path |
| [Qdrant snapshot / compaction](./qdrant-snapshot-compaction-contract.md) | Official snapshot API plus volume tar; no second vector DB |
| [Logging driver journald](./logging-driver-journald-contract.md) | json-file default; opt-in journald without Loki |
| [Helper Compose watch](./helper-compose-watch-contract.md) | Bind-mount first; optional `develop.watch` |
...
| Helper image | In-repo Alpine Dockerfile (processor pattern) | apk/pip on every start or a second API |
| Platform Postgres | `pg_dump` + volume tar; opt-in WAL | Patroni / hosted Postgres / sharing AFFiNE |
| Image authenticity | Official `cosign` on existing digests | Private Harbor as a prerequisite |
| Secrets mount | `.env` default; Compose `secrets:` + `*_FILE` | Vault / SOPS as a clone requirement |
| Qdrant day-2 | Official snapshot API + volume tar | Chroma / a second vector product |
| Container logs | Daemon `json-file`; opt-in journald | Loki/ELK as Tier 1 |
| Helper live edits | Bind-mount; optional Compose Watch | Flask debug or a sync sidecar |
| Image CVEs / pins | Stage 4 Trivy + Stage 15 safe-update | Treating cosign or Dependabot as CVE scanner |
...
2. Day-to-day tools → helper bake / Compose watch / Postgres WAL
3. Provenance / Compose secrets → Qdrant snapshots → journald
...
Extend `stage-merge-playbook.md` through **Stage 22** when reconciling hubs so sibling pages from PRs #27–#47 are not lost.
