...
description: Publishable Stage 25 community website markdown for Compose stop grace, tini PID 1, non-root users, processor poll energy, MCP pids_limit, per-container mem_swappiness, and an honest Tika healthcheck—extending Stages 1–24 without reinventing frameworks or community intake.
...
**Stage 25** (this run) adds **shutdown-drain, PID-1, identity, poll-energy, fork-cap, cgroup-swap, and Tika-probe constructional contracts** that remained open after Stage 1–24 drafts and consolidation PRs (#23–#50):

- Compose `stop_grace_period` for SIGTERM drain on stateful services
- `init: true` (tini) on first-party Python workers
- Numeric `user:` on stateless services, with a helper docker.sock exception
- Energy-honest `CHECK_INTERVAL` for the MinIO processor
- `pids_limit` on MCP tool sandboxes
- Per-container `mem_swappiness` (distinct from host sysctl)
- A real Tika HTTP healthcheck so Stage 24 `service_healthy` can apply

These are **constructional**: they tell contributors how to extend existing Compose keys, official Tika `/tika`, Docker tini, and cgroup knobs without inventing a shutdown orchestrator, supervisord, cluster IAM, an ingest bus, gVisor, `tuned`, or a dummy `exit 0` probe.
...
| Page | Purpose |
|---|---|
| [Compose stop grace period](./compose-stop-grace-period-contract.md) | SIGTERM drain on postgres/qdrant/minio/tika |
| [Compose init / tini](./compose-init-tini-contract.md) | `init: true` on helper and processor |
| [Compose non-root user](./compose-nonroot-user-contract.md) | Numeric `user:` on stateless first-party services |
| [Processor check interval energy](./processor-check-interval-energy-contract.md) | Honest `CHECK_INTERVAL` vs 10s poll |
| [MCP pids_limit sandbox](./mcp-pids-limit-sandbox-contract.md) | Fork caps on MCP tools and browserless |
| [Compose mem_swappiness](./compose-mem-swappiness-contract.md) | Per-container swap policy vs host sysctl |
| [Tika healthcheck honesty](./tika-healthcheck-honesty-contract.md) | Real `/tika` probe so Stage 24 can use `service_healthy` |
...
| Shutdown drain | Compose `stop_grace_period` + image SIGTERM | A shutdown sidecar or orchestrator |
| PID 1 | Compose `init: true` (engine tini) | supervisord / s6 / custom init images |
| Container identity | Numeric `user:` on stateless services | LDAP/Keycloak; blind `user: 1000` on helper |
| Processor poll | Existing `CHECK_INTERVAL` env | Kafka / Redis streams ingest bus |
| MCP fork cap | Compose `pids_limit` | gVisor / Kata for this gap |
| Swap policy | Per-container `mem_swappiness` + optional host sysctl | `tuned` / disable-swap-globally FAQ |
| Tika liveness | Official HTTP `/tika` | Dummy `exit 0` or `pgrep java` |
| Image CVEs / pins | Stage 4 Trivy + Stage 15 safe-update | Treating shutdown timers as supply-chain |
...
2. Day-to-day tools → stop grace / tini / non-root user
3. Processor poll → MCP pids → mem_swappiness → Tika probe
...
Extend `stage-merge-playbook.md` through **Stage 25** when reconciling hubs so sibling pages from PRs #27–#50 are not lost.
