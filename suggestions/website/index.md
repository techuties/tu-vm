...
description: Publishable Stage 17 community website markdown for weekly stack update, check-config, helper-contract-check, pre-commit hooks, release-drafter, security reporting, and changelog-refresh contracts that extend Stages 1–16 without reinventing frameworks or community intake.
...
TU-VM already has strong operational surfaces (`tu-vm.sh`, helper API, Nginx dashboard, playbooks, CI, smoke/pre-push gates, daily checkup, weekly stack update, check-config, helper-contract-check, pre-commit, Release Drafter, SECURITY.md, Dependabot, CODEOWNERS, and stale/triage automation). Historical suggestion branches repeatedly asked for the same foundations:
...
**Stage 17** (this run) adds **release, security, and local-gate glue contracts** that remained open after Stage 1–16 drafts and consolidation PRs (#23–#42):

- Weekly stack update contribution contract (`scripts/weekly-stack-update.sh` + sudoers example)
- Check-config contribution contract (`scripts/check-config.sh`)
- Helper-contract-check contribution contract (`scripts/helper-contract-check.sh`)
- Pre-commit hooks contribution contract (`.pre-commit-config.yaml`)
- Release Drafter contribution contract (`.github/release-drafter.yml` + workflow)
- Security reporting contribution contract (`SECURITY.md` + private advisory path)
- Changelog-refresh contribution contract (`scripts/changelog-refresh.sh`)
...
| Stage 1–16 website pages (open PRs #27–#42) | Contracts and living artifacts—implement, do not rewrite |
...
| Page | Purpose |
|---|---|
| [Weekly stack update contribution contract](./weekly-stack-update-contribution-contract.md) | Cron-friendly full-stack update lane distinct from daily checkup and safe-update digests |
| [Check-config contribution contract](./check-config-contribution-contract.md) | Secret-safe preflight for compose/env/allowlist without a second config product |
| [Helper-contract-check contribution contract](./helper-contract-check-contribution-contract.md) | Live helper JSON probes when `ai_helper_index` is up |
| [Pre-commit hooks contribution contract](./pre-commit-hooks-contribution-contract.md) | Optional local hooks wrapping shared syntax gates |
| [Release Drafter contribution contract](./release-drafter-contribution-contract.md) | Label-driven draft releases without a second changelog bot |
| [Security reporting contribution contract](./security-reporting-contribution-contract.md) | Private advisory-first vulnerability path for community reporters |
| [Changelog-refresh contribution contract](./changelog-refresh-contribution-contract.md) | Local Unreleased section helper distinct from Release Drafter |
...
| Scheduled health | Stage 16 `daily-checkup.sh` + host cron | Vendor RMM / second health platform |
| Weekly full update | `weekly-stack-update.sh` + narrow sudoers | Unattended root shells / broad NOPASSWD |
| Config preflight | `check-config.sh` | Second config linter product |
| Live helper probes | `helper-contract-check.sh` when helper is up | Always-on remote contract SaaS |
| Local hooks | Optional `.pre-commit-config.yaml` | Mandatory heavy hooks that duplicate CI minutes |
| Draft releases | Release Drafter + PR labels | Parallel changelog bots |
| Vulnerability reports | GitHub Security Advisories + `SECURITY.md` | Public exploit issues by default |
| Local Unreleased notes | `changelog-refresh.sh` | Overwriting Release Drafter categories |
...
2. Day-to-day tools → check-config / helper-contract-check / pre-commit / weekly update
3. Release Drafter → changelog-refresh → security reporting → CODEOWNERS → Dependabot
...
Extend `stage-merge-playbook.md` through **Stage 17** when reconciling hubs so sibling pages from PRs #27–#42 are not lost.
