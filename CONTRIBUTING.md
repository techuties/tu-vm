...
3. **Docs link job** — Workflow **Docs links** runs on **every** push/PR to `main` / `dev` and lists scanned files in the run **Summary**. Lychee checks links in those files (including same-repo GitHub URLs). Compare-range URLs and `user-attachments` screenshot links are excluded to avoid flaky CI.
...
- [`.github/workflows/docs-links.yml`](.github/workflows/docs-links.yml) — lychee on core Markdown (same-repo and external URLs checked; `/compare/` patterns and `user-attachments` excluded for stability).
